...
import gRETHlockchain from '../lib/ethereum.js';
...
export default function Home() {
  ...
  useEffect(() => {
    const init = async () => {
      try { 
        const { airdrop, accounts } = await gRETHlockchain();
        setAirdrop(airdrop);
    ...
    try {
      ...
      setClaimMessage({
        ...
        payload: `
          ...
          Total Amount: ${Web3.utils.fromWei(response.data.totalAllocation.toString())} RETH
          -> Basic allocation: ${Web3.utils.fromWei(response.data.basicAllocation.toString())} RETH
          -> Bonus allocation: ${Web3.utils.fromWei(response.data.bonusAllocation.toString())} RETH
        `
      });
            ...
            <div className="card-body">
              ...
              <div className="card-text">
                <ul>
                  ...
                  <li>Step 4: Confirm the transaction to claim your RETH tokens. This will send a transaction to the Airdrop smart contract</li>
                </ul>
              </div>
            </div>
          </div>
